...
/**
 * Xử lý các truy vấn dẫn xuất bằng cách phân tích tên phương thức repository.
 * Lớp này chuyển đổi tên phương thức thành một VMCQueryBuilder tương ứng.
 * @author NatswarChuan
 */
public class DerivedQueryHandler {
  ...
  private int applyCondition(
    ...
    if (segment.endsWith("Like")) {
      ...
      argsConsumed = 0;
      operator =
          segment.endsWith("IsNull") ? VMCSqlOperator.IS_NULL : VMCSqlOperator.IS_NOT_NULL;
    ...
    if (operator == VMCSqlOperator.IN && !(value instanceof Collection)) {
        throw new IllegalArgumentException("IN operator requires a Collection argument for field " + finalFieldName);
    }
